...
Current project status and GUI readiness are tracked in [docs/project-status-zh.md](docs/project-status-zh.md). GUI v1 usage, scope, service boundaries, and visual direction are documented in [docs/gui-v1-user-guide-zh.md](docs/gui-v1-user-guide-zh.md), [docs/gui-v1-scope-zh.md](docs/gui-v1-scope-zh.md), [docs/gui-v1-service-interface-zh.md](docs/gui-v1-service-interface-zh.md), and [docs/gui-v1-visual-design-brief-zh.md](docs/gui-v1-visual-design-brief-zh.md).
...
Door recognition reads width and height from common block attributes such as `WIDTH`, `HEIGHT`, `门宽`, and `门高` when available, and falls back to block scale or geometry. Geometric door lines and closed outlines can also read `HEIGHT` from `CAD_BUDGET` XDATA. Door count and door opening area are recorded when width and height are available. Door area is not deducted from wall area by default. Missing door-opening height defaults to 2.2m for quote trim/opening calculations, and missing sliding-door height also defaults to the residential quote rule value, currently 2.2m; designers can edit the generated Excel values.
...
cad-budget check-prices unit-prices.xlsx
