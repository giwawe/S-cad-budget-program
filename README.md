...
Door recognition reads width and height from common block attributes such as `WIDTH`, `HEIGHT`, `门宽`, and `门高` when available, and falls back to block scale or geometry. Door count and door opening area are recorded when width and height are available. Door area is not deducted from wall area by default. Missing door-opening height defaults to 2.2m for quote trim/opening calculations, while missing sliding-door height defaults to 2.4m; designers can edit the generated Excel values.
