...
Demolition and new masonry wall markers are explicit CAD opt-ins. `QUOTE_DEMO_WALL` linework produces demolition wall area by length times marker/default height. `QUOTE_NEW_WALL` linework uses `CAD_BUDGET` XDATA or block attributes `THICKNESS` and optional `HEIGHT`; 120mm and 240mm walls feed the matching quote lines. Brick-wall door/window lintels remain designer-entered because only newly added openings need lintels. Concrete lintel holes are quoted from the house-area proxy rather than CAD hole markers.
