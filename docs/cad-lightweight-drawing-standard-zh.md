...
| `QUOTE_LINTEL` | 过梁可选 | `LINE`、`LWPOLYLINE` | 标识砖墙门窗洞过梁位置 | 过梁支数 |
| `QUOTE_LINTEL_HOLE` | 过梁孔可选 | `POINT`、`INSERT`、`LINE`、`LWPOLYLINE` | 标识混凝土过梁孔位置 | 过梁孔个数 |
...
- `QUOTE_LINTEL` 用于标识砖墙门窗洞过梁，按标识数量计支数。
- `QUOTE_LINTEL_HOLE` 用于标识打混凝土过梁孔，支持点、块、线或多段线，按标识数量计个数。
- 没有这些标识时，拆改及拆墙、砌 120/240 厚砖墙、砖墙门窗洞过梁、打混凝土过梁孔仍按模板默认生成。
...
- 非空间模板项可通过规则文件自动汇总：室内地面面积、地砖加湿区墙砖面积、有效空间数量、湿区数量、厨房数量、卫生间数量、窗数量、窗面积、门洞数量、门洞面积、全屋固定数量、窗帘墙长、瓷砖片数、瓷砖加工费面积、普通室内门数量、宽门洞推拉门面积、推拉门套线长度、外墙净面积、拆墙面积、新砌墙面积、过梁数量、过梁孔数量、全屋定制投影面积、橱柜长度。
...
- `拆改及拆墙` 默认按 `QUOTE_DEMO_WALL` 面积汇总；`砌120厚砖墙` / `砌240厚砖墙` 默认按 `QUOTE_NEW_WALL` 面积并按 `THICKNESS` 匹配；`砖墙门窗洞过梁` 和 `打混凝土过梁孔` 默认按对应标识数量汇总。
