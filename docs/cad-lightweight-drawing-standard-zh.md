...
- 没有这些标识时，管道/包管类项目默认按 0 生成，并在复核备注中提示设计师手工输入；不从厨房或卫生间数量推断。
...
- 非空间模板项可通过规则文件自动汇总：室内地面面积、地砖加湿区墙砖面积、有效空间数量、湿区数量、厨房数量、卫生间数量、窗数量、窗面积、门洞数量、门洞面积、全屋固定数量、窗帘墙长、瓷砖片数、瓷砖加工费面积、普通室内门数量、入户门默认 0、宽门洞推拉门面积、推拉门套线长度、外墙净面积、外墙修补范围面积、拆墙面积、新砌墙面积、管道/包管立管长度、建筑面积百分比取整数、背景墙面积、玻璃淋浴房数量、蹲坑数量、全屋定制投影面积、地柜/吊柜长度。
