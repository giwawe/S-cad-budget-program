...
| `QUOTE_WALL_CABINET` | 橱柜推荐 | `LINE`、`LWPOLYLINE` | 画吊柜投影长度；转角柜建议画折线或闭合轮廓 | 吊柜长度明细复核 |
...
| `QUOTE_WALL_TILE` | 局部墙砖可选 | `LINE`、`LWPOLYLINE` | 画阳台、露台等非湿区的局部墙面贴砖范围，可写 `HEIGHT` | 局部墙砖面积，并并入美缝和墙面瓷砖片数 |
| `QUOTE_BACKGROUND_WALL` | 背景墙可选 | `LINE`、`LWPOLYLINE` | 画明确需要背景墙报价的墙面范围，可写 `HEIGHT` | 背景墙面积 |
| `QUOTE_SHOWER_GLASS` | 玻璃淋浴房可选 | `POINT`、`INSERT`、`LINE`、`LWPOLYLINE` | 每个标识代表 1 个玻璃淋浴房 | 玻璃淋浴房数量；未标识时默认为 0 |
| `QUOTE_SQUAT_TOILET` | 蹲坑可选 | `POINT`、`INSERT`、`LINE`、`LWPOLYLINE` | 每个标识代表 1 个蹲坑 | 蹲坑数量；未标识时默认为 0 |
...
- 闭合窗洞轮廓会从最小旋转矩形的主尺寸推断窗宽；相互接触的细长闭合窗洞会合并为 L 形窗，窗宽按各段窗洞长度之和计算。
...
- 窗面积会从墙面毛面积中扣除。
- `QuantityResult` 会保留 `window_details`，记录每个窗的宽、高、面积、是否默认窗高，以及最接近的房间墙段和墙段长度；报价中的窗帘/暗窗帘箱可使用窗所在墙面或窗洞长度汇总，报价复核状态和备注会把默认窗高异常汇总为中文数量提示，并只在墙面净面积、墙砖等受窗面积影响的空间项目上显示。
...
- 门洞缺少高度时，原有 `door_opening_area` 仍不计该门洞面积；报价明细中普通门洞/门套和推拉门面积默认按当前商品房报价规则高度计算，内置默认值均为 2.2m，并标记默认推断，设计师可在 Excel 中修改。
- 第一版默认不从墙面净面积中扣除门洞面积。
- 共享边界上的门会计入相邻空间。
...
- 柜体用开放折线时按折线长度汇总；用闭合轮廓时按占地面积除以推断柜深折算长度，避免 L 形转角柜只按外包投影长计。
...
- `QUOTE_EXT_REPAIR` 只用于明确需要修补的外墙范围，不从整面外墙自动推断。闭合且有面积的 `LWPOLYLINE` 直接按轮廓面积计算；开放 `LINE` / `LWPOLYLINE` 按线长乘 `HEIGHT`，缺少 `HEIGHT` 时按楼层默认高度或项目默认高度推断并提示复核。
...
- `QUOTE_LINTEL` 用于明确需要计入砖墙门窗洞过梁的新增门窗洞位置；每个标识按 1 支过梁计入报价。没有 `QUOTE_LINTEL` 时，砖墙门窗洞过梁默认按 0 生成，避免从普通门窗洞硬推；如需过梁由设计师手工填写。
...
## 12. 管道与包管标识
- `QUOTE_PIPE_INSULATION` 用于需要排污管包隔音棉的厨房、卫生间立管。
- `QUOTE_PIPE_WRAP` 用于需要包上/下水管道的单管立管。
- 推荐使用 `POINT` 或管道块 `INSERT` 标识立管平面点位；也支持 `LINE` / `LWPOLYLINE` 作为特殊长度标识。
- 可在 `CAD_BUDGET` XDATA 或块属性中写 `HEIGHT`，支持 `2400`、`2400mm`、`2.4`、`2.4m` 等写法。
- 缺少 `HEIGHT` 时，系统按楼层默认高度或项目默认高度生成计价长度，并在报价复核列标记默认推断。
- 报价中 `厨房、卫生间排污管包隔音棉` 默认按 `QUOTE_PIPE_INSULATION` 立管长度汇总；`包上/下水管道(单管)` 默认按 `QUOTE_PIPE_WRAP` 立管长度汇总。
- 没有这些标识时，管道/包管类项目默认按厨房和卫生间空间的层高合计乘以报价规则 `pipe_default_length_factor` 生成；内置默认系数为 1.5，并在复核备注中提示设计师可修改；有明确 `QUOTE_PIPE_INSULATION` / `QUOTE_PIPE_WRAP` 标识时优先使用标识长度。

## 13. 局部墙砖标识 `QUOTE_WALL_TILE`

- `QUOTE_WALL_TILE` 用于阳台、露台、洗衣区等非厨房、非卫生间空间的局部墙砖范围。
- 厨房、卫生间、主卫、公卫等湿区仍使用湿区墙砖规则：`墙面计量周长 * 2.5m - 窗面积`。湿区内即使画了 `QUOTE_WALL_TILE`，报价也不会额外叠加，避免重复计算。
- 使用 `LINE` 或 `LWPOLYLINE` 画需要贴砖的墙面长度，系统按 `线长 * HEIGHT` 计算局部墙砖面积。
- 可在 `CAD_BUDGET` XDATA 或块属性中写 `HEIGHT`，支持 `1200`、`1200mm`、`1.2`、`1.2m` 等写法。
- 缺少 `HEIGHT` 时，系统按匹配空间的当前层高或项目默认层高计算，并在报价复核列标记默认推断。
- 报价中 `墙面瓷砖` 片数和 `美缝` 会把非湿区 `QUOTE_WALL_TILE` 面积加到湿区墙砖面积中。

## 14. 背景墙标识 `QUOTE_BACKGROUND_WALL`

- `QUOTE_BACKGROUND_WALL` 用于明确需要按背景墙项目报价的墙面范围，不从普通墙面净面积自动推断。
- 使用 `LINE` 或 `LWPOLYLINE` 画背景墙长度，系统按 `线长 * HEIGHT` 计算背景墙面积。
- 可在 `CAD_BUDGET` XDATA 或块属性中写 `HEIGHT`，支持 `2400`、`2400mm`、`2.4`、`2.4m` 等写法。
- 缺少 `HEIGHT` 时，系统按匹配空间的当前层高或项目默认层高计算，并在报价复核列标记默认推断。
- 没有 `QUOTE_BACKGROUND_WALL` 时，`背景墙` 报价行默认按 0 生成，供设计师按客户方案手工填写。

## 15. 玻璃淋浴房标识 `QUOTE_SHOWER_GLASS`

- `QUOTE_SHOWER_GLASS` 用于明确需要按 `玻璃淋浴房` 项目报价的淋浴房，不从卫生间数量自动推断。
- 支持 `POINT`、`INSERT`、`LINE`、`LWPOLYLINE`；每个实体计为 1 个玻璃淋浴房。
- 该项目按个数报价，不按线长或面积报价；线段/折线只作为平面位置或范围提示。
- 没有 `QUOTE_SHOWER_GLASS` 时，`玻璃淋浴房` 报价行自动数量为 0，避免和按卫生间数量汇总的 `淋浴隔断` 重复报价。

## 16. 蹲坑标识 `QUOTE_SQUAT_TOILET`

- `QUOTE_SQUAT_TOILET` 用于明确需要按 `蹲坑` 项目报价的洁具，不从卫生间数量自动推断。
- 支持 `POINT`、`INSERT`、`LINE`、`LWPOLYLINE`；每个实体计为 1 个蹲坑。
- 该项目按个数报价，不按线长或面积报价；线段/折线只作为平面位置或范围提示。
- 没有 `QUOTE_SQUAT_TOILET` 时，`蹲坑` 报价行自动数量为 0，避免和按卫生间数量汇总的 `马桶` 重复报价。
...
- 报价 Excel 顶部的 `装修面积` 使用 `QuantityResult.building_area`；真实样例中该值来自 `QUOTE_EXT_WALL` 外框面积。表尾保留当前报价草稿的 `编制说明` 和签名栏。
- 可用 `cad-budget export-prices quote.xlsx --excel-output unit-prices.xlsx` 从报价表或模板导出全局单价表；也可用 `cad-budget init-prices template.xlsx` 初始化项目默认单价表 `config\quote-unit-prices.xlsx`，命令不会覆盖已有文件。单价表按精确 `项目名称 + 单位` 去重，同名同单位项目只维护一次主材、辅材、人工单价。预算员改价后先运行 `cad-budget check-prices unit-prices.xlsx`，空单价、非数字单价、同名同单位不同价格会阻断。后续 `cad-budget quote ... --unit-prices unit-prices.xlsx` 会用这张表覆盖模板单价；如果没有传 `--unit-prices` 且当前目录存在 `config\quote-unit-prices.xlsx`，`quote` 会自动使用默认单价表，因此新增方案和重复空间项目会套用同一套预算员确认单价。
- 可用 `cad-budget priced-quote result.json --template template.xlsx --output-dir priced-output` 一次生成正式报价输出包；命令要求存在有效单价表（显式 `--unit-prices` 或默认 `config\quote-unit-prices.xlsx`），会先校验单价，再输出 `quote-priced.xlsx`、`quote-priced-review.md`、`quote-priced-review.json` 和 `quote-priced-review-checklist.xlsx`。
...
- 非空间模板项可通过规则文件自动汇总：室内地面面积、建筑外框面积、地砖加湿区墙砖面积、有效空间数量、湿区数量、厨房数量、卫生间数量、窗数量、窗面积、门洞数量、门洞面积、全屋固定数量、窗帘墙长、瓷砖片数、瓷砖加工费面积、普通室内门数量、入户门默认 0、宽门洞推拉门面积、推拉门套线长度、外墙长度乘层高面积、外墙修补范围面积、拆墙面积、新砌墙面积、管道/包管立管长度或湿区默认长度、建筑面积百分比取整数、背景墙面积、玻璃淋浴房数量、蹲坑数量、全屋定制投影面积、地柜/吊柜长度。
- `房间成品保护` 默认按计入报价的有效室内空间数量汇总，避免保留模板占位数量。
...
- `暗窗帘箱` 只随有窗的普通干区空间生成，厨房、卫生间、阳台、露台等空间不生成暗窗帘箱；默认按同一房间同一墙段去重后的窗所在长度计算，L 形窗洞会按相接窗洞长度之和合并为一个窗。`窗帘` 仍作为非空间汇总项保留，按窗所在墙面长度汇总。
- `地面瓷砖` 使用计入的地砖铺贴面积，`墙面瓷砖` 使用湿区墙砖面积加非湿区 `QUOTE_WALL_TILE` 明确标识的局部墙砖面积；报价从模板项目名或说明中解析 `750X1500`、`600x1200` 等规格，按默认 5% 损耗向上取整为片数。
- `瓷砖加工费` 默认按房子面积计算，使用闭合 `QUOTE_EXT_WALL` 围合得到的 `QuantityResult.building_area`；如果外墙不能闭合，可用闭合 `QUOTE_BUILDING_AREA` 作为备用来源，没有建筑面积来源时保持模板默认。
- `垃圾清运费`、`材料搬运费`、`强电布线`、`弱电布线`、`水路布管`、`水泥墙开槽`、`补线、管槽及零星修补` 默认按 `QuantityResult.building_area` 建筑外框面积汇总；`地面砖现场维护费` 仍按计入的室内地面面积汇总。
- `室内门` 默认按唯一普通门洞数量汇总，不按面积；只从卧室、主卧、次卧、客卧、儿童房、老人房、书房等普通房间统计，宽度大于等于 1.4m 的大门洞和与卫生间共享的门洞不计入室内门。
- `厨房推拉门` 默认按厨房/书房中宽度大于等于 1.4m 的唯一门洞面积汇总；推拉门门高缺少时默认按 2.2m 计算。
- `厨房推拉门双包套` 默认按同一批推拉门门洞的套线长度汇总，单个门洞长度为门洞宽度加两侧有效门洞高度；门洞高度缺少时默认按 2.2m 计算。
- `阳台推拉门` 和 `阳台推拉门双包套` 使用独立的阳台/露台空间关键词，不会放宽厨房推拉门的匹配范围；前者按门洞面积，后者按门洞宽度加两侧有效门高；没有匹配阳台/露台宽门洞时按 0 生成。
- `外墙批嵌` 默认按选中的 `exterior_rows.gross_area` 汇总，即 `QUOTE_EXT_WALL` 长度乘以层高，不扣外墙洞口；`外墙批嵌以及修补` 默认按 `QUOTE_EXT_REPAIR` 明确标识的修补范围面积汇总，没有修补标识时按 0 生成并提示设计师手工输入，避免把修补范围等同于整面外墙。
- `拆改及拆墙` 默认按 `QUOTE_DEMO_WALL` 面积汇总，没有拆墙标识时按 0 生成；`砌120厚砖墙` / `砌240厚砖墙` 默认按 `QUOTE_NEW_WALL` 面积并按 `THICKNESS` 匹配，未填 `THICKNESS` 的新砌墙按 240mm 归类；明确只有其他厚度时，未匹配厚度报价按 0 自动生成；如果存在接近但未精确匹配的厚度（如 125mm 对 120mm），则提示确认 `THICKNESS`；`砖墙门窗洞过梁` 默认按 `QUOTE_LINTEL` 标识数量汇总，没有标识时按 0 生成，由设计师按需填写；`打混凝土过梁孔` 默认按 `QuantityResult.building_area` 的 10% 取整数，没有建筑面积来源时保持模板默认。
- `厨房、卫生间排污管包隔音棉` 默认按 `QUOTE_PIPE_INSULATION` 立管长度汇总；`包上/下水管道(单管)` 默认按 `QUOTE_PIPE_WRAP` 立管长度汇总，缺少高度时按楼层/项目默认高度推断并提示复核；没有对应标识时按厨房和卫生间空间的层高合计乘以报价规则 `pipe_default_length_factor` 默认生成，内置默认系数为 1.5，并提示设计师可修改。
- `淋浴隔断` 默认按卫生间数量汇总；`玻璃淋浴房` 按 `QUOTE_SHOWER_GLASS` 标识个数汇总，未标识时自动为 0，避免重复报价。
- `马桶` 默认按卫生间数量汇总；`蹲坑` 按 `QUOTE_SQUAT_TOILET` 标识个数汇总，未标识时自动为 0，避免重复报价。
- `全屋定制` 默认从 `QUOTE_CUSTOM` 投影面积汇总；缺少高度时按 2.6m 默认推断，高度低于 1m 的柜体不计入投影面积并提示按长度复核。
- `地柜` / `吊柜` 默认从 `QUOTE_BASE_CABINET` / `QUOTE_WALL_CABINET` 长度分别汇总；旧图可继续用 `QUOTE_CABINET + TYPE` 区分类型。若模板只有通用 `橱柜` 行，报价导出会自动拆成 `地柜` 和 `吊柜` 两行；若模板已有分项，则跳过通用 `橱柜` 行以避免重复报价。显式地柜/吊柜图层会自动写入类型，地柜和吊柜可重叠标注；开放折线按折线长度计，闭合柜体轮廓按占地面积除以推断柜深折算长度，避免转角柜漏算折段。
- `背景墙` 默认从 `QUOTE_BACKGROUND_WALL` 面积汇总；没有背景墙标识时按 0 生成，避免把普通墙面硬推为背景墙，由设计师按客户方案填写。
- `入户门` 默认按 0 生成，因为入户门经常不在报价范围内；如客户要求更换，由设计师手工填写。
- `砖墙门窗洞过梁`、`背景墙`、`入户门`、`玻璃淋浴房`、`蹲坑` 这类已确认按需选择项在未标识时按 0 自动生成，并保留设计师复核备注；它们不占用 `自动生成-默认推断` 状态，以便默认推断列表集中反映窗高、门高、墙高、建筑面积来源等计算假设。
...
- 需要自动报价背景墙时，已使用 `QUOTE_BACKGROUND_WALL` 标识背景墙范围，并尽量填写 `HEIGHT`；没有背景墙方案时可不画，报价默认 0。
- 需要自动报价玻璃淋浴房时，已使用 `QUOTE_SHOWER_GLASS` 标识每个玻璃淋浴房；不需要玻璃淋浴房时不要画该图层，系统会按 0 处理。
- 需要自动报价蹲坑时，已使用 `QUOTE_SQUAT_TOILET` 标识每个蹲坑；不需要蹲坑时不要画该图层，系统会按 0 处理。
...
| `QUOTE_WALL_TILE` 缺少 `HEIGHT` | 按匹配空间当前层高或项目默认层高计算，并在报价复核列标记默认推断 |
| `QUOTE_BACKGROUND_WALL` 缺少 `HEIGHT` | 按匹配空间当前层高或项目默认层高计算，并在报价复核列标记默认推断 |
| 未绘制 `QUOTE_SHOWER_GLASS` | `玻璃淋浴房` 数量默认为 0，避免和 `淋浴隔断` 重复报价 |
| 未绘制 `QUOTE_SQUAT_TOILET` | `蹲坑` 数量默认为 0，避免和 `马桶` 重复报价 |
