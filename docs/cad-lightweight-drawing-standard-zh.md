...
- 可用 `cad-budget export-prices quote.xlsx --excel-output unit-prices.xlsx` 从报价表或模板导出全局单价表；该表按精确 `项目名称 + 单位` 去重，同名同单位项目只维护一次主材、辅材、人工单价。预算员改价后先运行 `cad-budget check-prices unit-prices.xlsx`，空单价、非数字单价、同名同单位不同价格会阻断。后续 `cad-budget quote ... --unit-prices unit-prices.xlsx` 会用这张表覆盖模板单价，因此新增方案和重复空间项目会套用同一套预算员确认单价。
