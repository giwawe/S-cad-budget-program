...
- 可用 `cad-budget quote-report quote.xlsx --quantity-json result.json --markdown-output quote-review.md --json-output quote-review.json` 从报价 Excel 的可见复核列生成 Markdown 复核报告，也可同步生成结构化 JSON；`--quantity-json` 可省略，传入时会补充房间/对象摘要，例如哪些空间存在默认窗高、门洞/推拉门高度或全屋定制高度/类型。报告顶部会按备注归纳补图/复核行动建议，例如补窗高、新砌墙高度/厚度、管道/包管标识、门洞/推拉门高度、全屋定制高度/类型，并显示每类行动影响的报价行数、涉及项目名摘要和 Excel 行号；JSON 会保留行动建议、优先级、建议动作、状态/来源统计和复核明细行；随后 Markdown 按 `自动生成-默认推断`、`自动生成-异常提示`、`按模板生成` 分组列出需要设计师/预算员复核的行，便于真实模板回归对比。
...
- 非空间模板项可通过规则文件自动汇总：室内地面面积、地砖加湿区墙砖面积、有效空间数量、湿区数量、厨房数量、卫生间数量、窗数量、窗面积、门洞数量、门洞面积、全屋固定数量、窗帘墙长、瓷砖片数、瓷砖加工费面积、普通室内门数量、入户门默认 0、宽门洞推拉门面积、推拉门套线长度、外墙长度乘层高面积、外墙修补范围面积、拆墙面积、新砌墙面积、管道/包管立管长度或湿区默认长度、建筑面积百分比取整数、背景墙面积、玻璃淋浴房数量、蹲坑数量、全屋定制投影面积、地柜/吊柜长度。
...
- `厨房推拉门` 默认按厨房/书房中宽度大于等于 1.4m 的唯一门洞面积汇总；推拉门门高缺少时使用默认推拉门高度 2.4m。
- `厨房推拉门双包套` 默认按同一批推拉门门洞的套线长度汇总，单个门洞长度为门洞宽度加两侧有效门洞高度；门洞高度缺少时默认 2.2m。
