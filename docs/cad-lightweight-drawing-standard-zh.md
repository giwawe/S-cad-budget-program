...
- 可在 `CAD_BUDGET` XDATA 或块属性中写 `HEIGHT`，支持 `2400`、`2400mm`、`2.4`、`2.4m` 等写法。
- 缺少 `HEIGHT` 时，系统按匹配空间的当前层高或项目默认层高计算，并在报价复核列标记默认推断。
- 没有 `QUOTE_BACKGROUND_WALL` 时，`背景墙` 报价行保持模板默认，供预算员按方案复核。
...
- 非空间模板项可通过规则文件自动汇总：室内地面面积、地砖加湿区墙砖面积、有效空间数量、湿区数量、厨房数量、卫生间数量、窗数量、窗面积、门洞数量、门洞面积、全屋固定数量、窗帘墙长、瓷砖片数、瓷砖加工费面积、普通室内门数量、宽门洞推拉门面积、推拉门套线长度、外墙净面积、外墙修补范围面积、拆墙面积、新砌墙面积、管道/包管立管长度、建筑面积百分比取整数、背景墙面积、玻璃淋浴房数量、蹲坑数量、全屋定制投影面积、地柜/吊柜长度。
...
- `背景墙` 默认从 `QUOTE_BACKGROUND_WALL` 面积汇总；没有背景墙标识时保持模板默认，避免把普通墙面硬推为背景墙。
...
- 需要自动报价背景墙时，已使用 `QUOTE_BACKGROUND_WALL` 标识背景墙范围，并尽量填写 `HEIGHT`。
