...
- 非空间模板项可通过规则文件自动汇总：室内地面面积、地砖加湿区墙砖面积、有效空间数量、湿区数量、厨房数量、卫生间数量、窗数量、窗面积、门洞数量、门洞面积、全屋固定数量。
...
- 门洞数量和门洞面积可用于报价汇总，但第一版仍不从墙面净面积中扣除门洞面积。
- 共享边界上的门可能计入相邻空间，涉及门洞数量汇总的报价项需要预算员复核。
