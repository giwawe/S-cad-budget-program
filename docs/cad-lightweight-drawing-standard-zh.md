...
- 非空间模板项可通过规则文件自动汇总：室内地面面积、地砖加湿区墙砖面积、有效空间数量、湿区数量、厨房数量、卫生间数量、窗数量、窗面积、门洞数量、门洞面积、全屋固定数量、窗帘墙长、瓷砖片数、瓷砖加工费面积、普通室内门数量、宽门洞推拉门面积、推拉门套线长度、外墙净面积、外墙修补范围面积、拆墙面积、新砌墙面积、管道/包管立管长度、建筑面积百分比取整数、全屋定制投影面积、地柜/吊柜投影长度。
...
- `地柜` / `吊柜` 默认从 `QUOTE_BASE_CABINET` / `QUOTE_WALL_CABINET` 长度分别汇总；旧图可继续用 `QUOTE_CABINET + TYPE` 区分类型。若模板只有通用 `橱柜` 行，报价导出会自动拆成 `地柜` 和 `吊柜` 两行；若模板已有分项，则跳过通用 `橱柜` 行以避免重复报价。显式地柜/吊柜图层会自动写入类型，地柜和吊柜可重叠标注；封闭或近似闭合的柜体轮廓按投影主长度计，不按轮廓周长计。
- 背景墙当前仍按模板或人工复核处理。
