...
- `拆改及拆墙` 默认按 `QUOTE_DEMO_WALL` 面积汇总，没有拆墙标识时按 0 生成；`砌120厚砖墙` / `砌240厚砖墙` 默认按 `QUOTE_NEW_WALL` 面积并按 `THICKNESS` 匹配，未填 `THICKNESS` 的新砌墙按 240mm 归类；有新砌墙标识但某一厚度未匹配时，该厚度报价按 0 生成并提示确认 `THICKNESS`；`砖墙门窗洞过梁` 默认按 `QUOTE_LINTEL` 标识数量汇总，没有标识时按 0 生成，由设计师按需填写；`打混凝土过梁孔` 默认按 `QuantityResult.building_area` 的 10% 取整数，没有建筑面积来源时保持模板默认。
