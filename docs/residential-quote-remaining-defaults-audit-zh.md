...
审计基于真实样例 `scratch/cad-import-10-squat-toilet-default/quote.xlsx`。当前样例统计为：
...
这些项目已经有自动化规则；当前真实样例保持模板默认，是因为 `scratch/cad-import-10-squat-toilet-default/result.json` 中没有对应 CAD 标识。后续设计师按规范补图层后即可自动汇总。
