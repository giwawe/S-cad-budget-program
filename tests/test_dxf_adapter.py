...
def test_imports_closed_cabinet_outline_using_footprint_length_not_bounding_projection(tmp_path: Path):
    doc = ezdxf.new("R2010")
    doc.header["$INSUNITS"] = 4
    modelspace = doc.modelspace()
    ...
    assert result.project is not None
    assert len(result.project.cabinet_items) == 1
    assert result.project.cabinet_items[0].length == pytest.approx(5.109, abs=0.001)


def test_imports_l_shaped_closed_cabinet_outline_using_footprint_length(tmp_path: Path):
    doc = ezdxf.new("R2010")
    doc.header["$INSUNITS"] = 4
    modelspace = doc.modelspace()
    for layer in ["QUOTE_ROOM", "QUOTE_TEXT", "QUOTE_BASE_CABINET"]:
    ...
    modelspace.add_lwpolyline(
        ...
        dxfattribs={"layer": "QUOTE_BASE_CABINET", "closed": True},
    )
    dxf_path = _save_doc(tmp_path / "l-cabinet-outline.dxf", doc)
    ...
    assert result.project is not None
    assert len(result.project.cabinet_items) == 1
    assert result.project.cabinet_items[0].length == pytest.approx(4.4)
