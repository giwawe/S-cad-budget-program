...
设计师可读的 CAD 出图轻量规范主文档是 `docs/cad-lightweight-drawing-standard-zh.md`。修改 `LayerName`、`dxf_adapter.py`、`quantity.py`、Excel/报价对 CAD 结果的使用口径，或新增/变更 CAD 图层、块属性、单位规则、几何推断规则、异常口径时，必须同步更新该文档。
...
- 厨房、卫生间、主卫、公卫等湿区空间用地面面积生成地面项目；墙面防水和墙砖使用专门高度规则。
- 湿区墙面项目不要直接用整墙 `net_wall_area`：厨房防水为 `地面面积 + 墙面计量周长 * 0.3m`；卫生间/主卫/公卫防水为 `地面面积 + 墙面计量周长 * 1.8m`；墙面贴瓷砖为 `墙面计量周长 * 2.5m - 窗面积`，不扣门洞。
- 默认商品房报价规则在 `src/cad_budget/config/residential_quote_rules.json`；`quote` 命令默认读取内置规则文件，也支持 `--rules rules.json` 指定外部规则文件。报价 Excel 的 `Q7:R7` 会记录规则来源。规则 JSON 格式错误、缺少必要字段或高度不是数字时应返回清晰错误，不输出 traceback。
- 可用 `cad-budget init-rules --output rules.json` 导出一份可修改的默认规则文件，再通过 `quote --rules rules.json` 使用。`init-rules` 默认不覆盖已有文件，避免误删预算规则。
- 主材、全屋定制、室内门、卫浴、水电、其他等当前 CAD 无法准确判断的项目保留模板默认数量和单价，供预算员复核修改。
- 报价表右侧有可见复核列：数量来源、来源空间、空间ID、计量口径、复核状态、复核备注。报价采用自动优先策略：`confirmed` / `manually_edited` 标记为 `自动生成`，`default_inferred` 标记为 `自动生成-默认推断` 并写入默认推断备注，`needs_review` 标记为 `自动生成-异常提示` 并写入异常备注，模板默认项标记为 `按模板生成`。默认推断和异常提示不会阻止报价生成；`excluded` 空间不生成报价分组。
- 非空间报价项优先自动汇总：垃圾清运费、材料搬运费、地面砖现场维护费、强电布线、弱电布线、水路布管、水泥墙开槽、补线/管槽及零星修补等按计入的室内地面面积汇总；`美缝` 按地砖面积加湿区墙砖面积汇总。门、卫浴、窗帘、全屋定制、瓷砖片数等缺少可靠 CAD 数据来源的项目继续按模板生成。
- 报价 Excel 的主表仍在 `A:O` 列；`Q:S` 列写入自动化统计区，统计 `自动算量`、`自动汇总`、`模板默认` 的行数和占比。
