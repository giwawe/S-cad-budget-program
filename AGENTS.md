...
- 公式字段：
  - 墙面毛面积：`墙面计量周长 * 层高`
  - 墙面净面积：`墙面毛面积 - 窗面积`
- JSON 输出仍是静态计算结果；Excel 的公式只服务人工复核和报价前修改，当前不会回写 JSON。
...
- 增强普通 CAD 文字兜底识别，目前实现主要依赖 `QUOTE_TEXT`。
- 增强门块属性解析，目前门块主要从 block scale 推断宽度。
- 外墙工程量需要独立结果表和 Excel 展示。
